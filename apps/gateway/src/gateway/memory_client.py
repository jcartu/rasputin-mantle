...
class MemoryClient:
    ...
    def __init__(self, memory_url: str | None = None, timeout: float = 10.0) -> None:
        ...
        if self.memory_url:
            try:
                async with httpx.AsyncClient(base_url=self.memory_url, timeout=self.timeout) as client:
                    response = await client.post("/commit", json={"key": key, "value": value, "text": str(value)})
    ...
    async def query(self, query: str, k: int = 5) -> dict[str, Any]:
        if self.memory_url:
            try:
                async with httpx.AsyncClient(base_url=self.memory_url, timeout=self.timeout) as client:
                    response = await client.get("/search", params={"q": query, "limit": k})
                    if 200 <= response.status_code < 300:
                        payload = response.json()
                        if isinstance(payload, dict):
                            payload.setdefault("backend", "rasputin-memory")
