...
function runBrowserUse(args: string[]): Promise<CommandResult> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(new BrowserActionError(`browser-use ${args.join(" ")} timed out after ${COMMAND_TIMEOUT_MS / 1000}s`));
    }, COMMAND_TIMEOUT_MS);
...
function parseState(stdout: string): BrowserState {
  ...
  for (const line of stdout.split("\n")) {
    ...
    elements.push({ id: match[1], role: match[2], text: match[3] ?? undefined, attributes: {} });
  }
...
export class BrowserUseBackend implements BrowserBackend {
  ...
  async getState(): Promise<BrowserState> {
    ...
    try {
      ...
      throw new BrowserActionError(`Failed to parse browser-use state: ${message}`);
    }
  }
