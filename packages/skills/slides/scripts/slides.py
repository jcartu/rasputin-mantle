...
TEMPLATE_DIR = SKILLS_ROOT / "slides" / "templates"
...
def _template_path(template: str) -> Path:
    candidate = TEMPLATE_DIR / f"{template}.pptx"
    if candidate.exists():
        return candidate
    return TEMPLATE_DIR / "minimal.pptx"


def _load_template_deck(template: str) -> Presentation:
...
def _text_shapes(slide: Any) -> list[Any]:
    return [shape for shape in slide.shapes if hasattr(shape, "text_frame")]


def _replace_text(shape: Any, text: str, style: dict[str, RGBColor], size: int, bold: bool = False) -> None:
...
def _populate_template_slides(prs: Presentation, outline: dict[str, Any], sections: list[Any], template: str, style: dict[str, RGBColor]) -> None:
    if len(prs.slides) < 5:
        return
    title = str(outline.get("title") or "Presentation")
    subtitle = str(outline.get("subtitle") or "Generated by Mantle Slides")
    title_shapes = _text_shapes(prs.slides[0])
    if title_shapes:
        _replace_text(title_shapes[0], title, style, 40, True)
    if len(title_shapes) > 1:
        _replace_text(title_shapes[1], subtitle, style, 18)
    _add_notes(prs.slides[0], str(outline.get("notes") or "Introduce the deck purpose and agenda."))

    agenda = [str(section.get("title") or f"Section {idx}") for idx, section in enumerate(sections, start=1)]
    divider_shapes = _text_shapes(prs.slides[1])
    if len(divider_shapes) > 1:
        _replace_text(divider_shapes[1], "Agenda", style, 36, True)
    if len(divider_shapes) > 2:
        _replace_text(divider_shapes[2], " • ".join(agenda[:5]) or subtitle, style, 16)

    content_shapes = _text_shapes(prs.slides[2])
    if content_shapes:
        _replace_text(content_shapes[0], "Key Discussion Areas", style, 26, True)
    for shape_index, section in zip((1, 3), sections[:2]):
        if len(content_shapes) > shape_index:
            _replace_text(content_shapes[shape_index], str(section.get("title") or "Section"), style, 16, True)
        if len(content_shapes) > shape_index + 1:
            bullets = "\n".join(f"• {item}" for item in list(section.get("bullets") or section.get("items") or [])[:3])
            _replace_text(content_shapes[shape_index + 1], bullets or "• Add supporting detail", style, 14)

    chart_shapes = _text_shapes(prs.slides[3])
    if chart_shapes:
        _replace_text(chart_shapes[0], "Evidence Snapshot", style, 26, True)
    if len(chart_shapes) > 2:
        takeaway = "\n".join(f"• {item}" for item in agenda[:3])
        _replace_text(chart_shapes[2], takeaway or "Summarize the most important trend.", style, 14)
    _add_footer(prs.slides[4], template, len(prs.slides), style)
...
def create_presentation(payload: dict[str, Any]) -> Path:
    ...
    prs = _load_template_deck(template)
    _populate_template_slides(prs, {**outline, "title": title}, sections, template, style)

    with tempfile.TemporaryDirectory() as tmp:
        tmp_dir = Path(tmp)
        for idx, section in enumerate(sections, start=len(prs.slides) + 1):
